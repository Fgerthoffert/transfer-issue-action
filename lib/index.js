  ...
  try {
    ...
    if (core.getInput('allow_private_public_transfer') === 'true') {
      ...
      if (sourceRepo.private === true && targetRepo.data.private === false) {
        ...
        core.info(`Giving current user Admin privileges: ${tempRepoName}`);
        const repoPerm = await octokit.rest.repos.addCollaborator({
          owner: sourceRepo.owner.login,
          repo: tempRepoName,
          username: 'jahia-ci',
          permission: 'admin'
        })      
        console.log('repoPerm', repoPerm)
