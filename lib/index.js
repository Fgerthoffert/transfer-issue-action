...
async function run() {
  try {
    ...
    const enableCustomLabelRouting = core.getInput('router', {required: false}) === 'true';
    ...
    if (enable_custom_label_routing) {
      const [ labelRouter, targetRepo ] = router.split(':')
    ...
    if (!enableCustomLabelRouting && triggerLabel && triggerLabel !== sourceLabel.name) {
