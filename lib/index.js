  ...
  try {
    ...
    if (core.getInput('allow_private_public_transfer') === 'true') {
      ...
      if (sourceRepo.private === true && targetRepo.data.private === false) {
        ...
        `);
        core.info(`Issue transferred to: ${transferDstRepo.transferIssue.issue.url}`);

        core.info(`Deleting temporary repository: ${tempRepoNamel}`);
        ...
        core.info(`Repository: ${tempRepoNamel} deleted`);
