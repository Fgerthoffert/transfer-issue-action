...
async function run() {
  try {
    ...
    // Split the source label into relevant pieces
    const srcLabelparts = router.split(':');
    const scrLabelDestination = parts[1];
    const scrLabelTrigger = parts[0];

    console.log('will transfer issue to %s if %s matches %s', targetRepoName, triggerLabel, scrLabelTrigger)
    console.log(parts)
    debug('will transfer issue to %s if %s matches %s', targetRepoName, triggerLabel, scrLabelTrigger);

    // If trigger label doesnt match the issue label then bail
    if (triggerLabel && triggerLabel !== scrLabelTrigger) {
      return core.notice(`Issue not transferred because "${scrLabelTrigger}" != "${triggerLabel}"!`);
    }
    ...
    if (core.getInput('allow_private_public_transfer') === 'true') {
      ...
      if (sourceRepo.private === true && targetRepo.data.private === false) {
        ...
        `);
        console.log('TransferToTmpRepo', transferToTmpRepo)
        ...
        const tempRepoPublic = await octokit.request('PATCH /orgs/{org}/{repo}', {
          org: sourceRepo.owner.login,
          repo: tempRepoName,
          'private': false,
          headers: {
        ...
        console.log('tempRepoPublic', tempRepoPublic)
        ...
        const transfer = await octokit.graphql(`
          mutation {
              transferIssue(input: {
                ...
                createLabelsIfMissing: ${core.getInput('create_labels_if_missing') === 'true'},
                issueId: "${transferIssueToTmpRepo.node_id}"
              }) {
                issue {
                  number
                }
        ...
        core.info(`Delete temporary repository`);
        const tempRepoDelete = await octokit.request('DELETE /orgs/{org}/{repo}', {
          org: sourceRepo.owner.login,
          repo: tempRepoName,
          headers: {
            'X-GitHub-Api-Version': '2022-11-28'
        ...
        console.log('tempRepoDelete', tempRepoDelete)
      ...
      console.log(core.getInput('create_labels_if_missing'))
      console.log(core.getInput('create_labels_if_missing') === 'true')
      console.log(targetRepo.data.node_id)
      console.log(sourceIssue.node_id)
      const transfer = await octokit.graphql(`
        mutation {
            transferIssue(input: {
              clientMutationId: "${crypto.randomBytes(20).toString('hex')}",
